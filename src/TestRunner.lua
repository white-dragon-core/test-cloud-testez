	...
	local function runCallback(callback, messagePrefix)
		...
		local nodeSuccess, nodeResult = xpcall(
			...
			function(message)
				...
				-- 改进 require() 错误消息，提供更多上下文
				if msg:find("Requested module experienced an error while loading") then
					-- 尝试从堆栈跟踪中提取更详细的信息
					...
					-- 构建增强的错误消息
					local enhancedMsg = msg
					if userCodeLine ~= "" then
						enhancedMsg = msg .. "\n  → Failed at: " .. userCodeLine:match("^%s*(.+)$")
					end

					return messagePrefix .. debug.traceback(enhancedMsg, 2)
				end

				return messagePrefix .. debug.traceback(msg, 2)
			end
		)
